...
export default function ArticleGenerator() {
  ...
  const generateArticle = async () => {
    ...
    try {
      ...
      content += sortedPts.map(pt => `- ${brand.name} ${model.name} ${formatPTName(pt.name)}`).join('\n') + "\n\n";
      ...
      sortedPts.forEach(pt => {
        ...
        if (lastFreeInterval) {
          ...
          content += `The periodic services for the ${brand.name} ${model.name} ${pName} carry a labour charge of up to Rs. ${new Intl.NumberFormat('en-IN').format(maxLabour)}.\n\n`;
        }
        ...
        tableHtml += `<tr class="header-row"><th class="label-col"><b>INTERVAL →</b></th>${ptIntervals.map(i => `<th><b>${getPeriodLabel(i.months)}</b></th>`).join('')}</tr>`;
        tableHtml += `<tr class="header-row"><th class="label-col"><b>ODO MTR →</b></th>${ptIntervals.map(i => `<th><b>${formatNumber(i.kilometers)}</b></th>`).join('')}</tr></thead>`;
        tableHtml += `<tbody><tr><td class="label-col"><b>Labour charges</b></td>${ptIntervals.map(i => `<td><b>${formatNumber(Math.round(i.laborCost || 0))}</b></td>`).join('')}</tr>`;
        
        itemsList.forEach(item => {
          tableHtml += `<tr><td class="label-col"><b>${item.displayName || "Maintenance Item"}</b></td>`;
          ptIntervals.forEach(interval => {
            ...
            tableHtml += `<td><b>${formatNumber(Math.round(val))}</b></td>`;
          });
          tableHtml += `</tr>`;
        });
        ...
        if (hasUnitemizedParts) {
          tableHtml += `<tr><td class="label-col"><b>Parts</b></td>${ptIntervals.map(i => {
            const hasItems = i.items && i.items.length > 0;
            const val = hasItems ? 0 : (i.partsCost || i.partsConsumablesCost || 0);
            return `<td><b>${formatNumber(Math.round(val))}</b></td>`;
          }).join('')}</tr>`;
        }
        
        tableHtml += `<tr class="total-charges-row"><td class="label-col"><b>Total Charges</b></td>${ptIntervals.map(i => `<td><b>${formatNumber(Math.round(i.totalCost || 0))}</b></td>`).join('')}</tr>`;
        ...
        content += `For the first 5 years of ownership, your average monthly maintenance cost will stand at Rs. ${formatNumber(avg5y)} with total costs adding up to Rs. ${formatNumber(cost5y.total)}. If you keep the ${model.name} ${pName} for ${longTermLabel}, you can expect to spend Rs. ${formatNumber(avgLong)} per month on maintenance. In this case, the total maintenance expense for long-term ownership will be Rs. ${formatNumber(costMaxY.total)}.\n\n`;
        ...
        content += `For the first ${formatNumber(cost75k.limit)}km of driving the ${brand.name} ${model.name} ${pName}, you will have to pay Rs. ${rate75k} per km in routine maintenance, amounting to a total of Rs. ${formatNumber(cost75k.total)}. By ${formatNumber(cost105k.limit)}km, the ${model.name} ${pName} will cost Rs. ${formatNumber(cost105k.total)} in periodic maintenance, which is Rs. ${rate105k} per kilometer. At the ${formatNumber(costMaxK.limit)}km mark, your per kilometre maintenance cost will increase to Rs. ${rateMax} per km with the total adding up to Rs. ${formatNumber(costMaxK.total)}.\n\n`;
      });
  ...
  const copyAsRichText = async () => {
    ...
    const tableStyles = `
      <style>
        table { border-collapse: collapse; width: 100%; max-width: 1280px; border: 1px solid #000; margin: 10px 0; font-family: Arial, sans-serif; background: #fff; line-height: 1.15; }
        tr { height: 30px; }
        ...
        p { line-height: 1.15; margin: 0; padding: 0; }
        h1 { font-size: 20px; font-weight: 700; margin: 0 0 10px 0; }
